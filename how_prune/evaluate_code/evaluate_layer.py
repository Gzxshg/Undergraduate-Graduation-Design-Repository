...
# 绑定到输出
sys.stdout = DoubleOutput("log.txt")

# 你原来所有print原样写
print("会同时打印在终端 + 写入txt",flush=True)
...
def run_strict_sensitivity_analysis(model, prune_ratio=0.2, device="cuda"):
    print(">>> 启动受限架构下的模块级敏感度分析...",flush=True)
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module, SUPPORTED_OPERATORS_WHITELIST):
            print(f"正在分析受支持层: {name} | 类型: {str(type(module).__name__)}",flush=True)
            ...
            try:
                ...
                print(f"  [!] 处理层 {name} 时发生异常: {e}",flush=True)
    ...
    print(f"分析完成。共测试了 {tested_layers_count} 个符合白名单规范的网络层。",flush=True)
...
if __name__ == "__main__":
    ...
    # 输出结果
    print(">>> 各层敏感度排名 (建议从顶部开始分配较高剪枝率)：",flush=True)
    for rank, (layer_name, metrics) in enumerate(results, 1):
        print(f" {rank}. {layer_name} ({metrics['layer_type']}) | 掉点: {metrics['metric_drop']:.4f}",flush=True)
    ...
    print("\n已导出规范化蓝图配置文件：mst_pruning_blueprint.json",flush=True)